...
//ADC_DMA回调，根据标志位判断cpu当前状态并执行（进行下轮ADC采样 or 停止采样，等待cpu绘图）
//ADC_DMAConvCplt
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc){
	...
	if(Sign_readyDisplay && Sign_samplingOver == 1){
		//cpu已经准备好绘图，DMA不再传输下轮数据，待cpu绘图结束后由cpu再次开启ADC-DMA
		//DMA选用normal模式，不再开启即关闭
		;
	}
	else{
		//cpu未准备好绘图，DMA传输下轮数据
